...
def main(database, table):
    ...
    if connectMysql.status:
        ...
        for choose_table in tqdm(all_table_from_mysql):
            if choose_table in identification_all_databases_from_mysql:
                continue
            try:
                ...
                continue
            output_dataframe_list_concat = per_run.run(choose_dataframe, table_name=choose_table, recall_mode=True)
...
if __name__ == '__main__':
    ...
    if connectMysql.status:
        ...
        print('您选择的是{}'.format(choose_database))
        connectMysql.selectDb(choose_database)
        choose_dataframe = connectMysql.query('information')
